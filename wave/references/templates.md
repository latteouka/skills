...
(3) 品質閘門通過：🔒 安全 skill 0 high/critical；typecheck 綠（exit 0）；🎨 UX 審計已執行並記錄，或依降級規則標「待 UX 補跑」封鎖閘門（完整品質閘門由 merge 後 deep lane 承擔，不在本條）
...
| (3) 品質閘門 | typecheck exit code + 安全/UX 閘門記錄（deep lane 全綠與否不入本條，紅燈走 inbox 開單） |
