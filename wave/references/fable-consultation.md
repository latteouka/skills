...
**不設收尾終檢諮詢點**：完工後再請一個模型複查交付物屬 over-verification，[官方 Opus 5 指引](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5)明列應移除。交付物真偽由合約實跑輸出與 typecheck exit code 認定（完整品質由 merge 後 deep lane 承擔），不由第二個模型的意見認定。**逐項 commit 前也不諮詢**——N 項 × 30-90 秒，且與 reviewer 親自重跑合約完全重複。
