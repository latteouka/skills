...
4. **Merge typecheck（強制，貼輸出才算合併完成）**：在 main 上依序跑 ① 專案 typecheck（~90 秒，同步）② 受影響 view 的 E2E 子集（本波改動檔案對應的 golden-path spec，不整套跑）——任何紅燈先修再宣告合併完成。理由：merge 殘留 TS 錯誤曾多次上 main。完整品質閘門不在此跑——由 merge 後 deep lane 承擔（`wave-gate.sh baseline`，紅燈寫 inbox 開單）
