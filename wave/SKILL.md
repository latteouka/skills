...
(2) 每項覆蓋 happy path + edge case + 誤用場景（合約內列的全部跑過）
(3) 品質閘門通過：🔒 安全 skill 0 high/critical，🎨 UX 審計已執行並記錄到 wave-{id}.md
...
1. **基線綠燈（dev mode；general mode 跳過）**——跑一次現有測試基線（輸出帶唯一標記字串確認是本輪跑出的），結果記進 ledger 第一行（例：`基線：153 檔 / 1503 tests passed`）。之後任何紅燈都可歸因是本波引入還是既存問題
...
- 安全 0 high/critical + UX 審計已跑完記錄 → 品質閘門通過，進入收尾流程（標「✅ 完成」前仍須通過收尾稽核 subagent）
- 安全有 high/critical 殘留 → **不可標「✅ 完成」**，回去修
...
2. **資料正確性** — 斷言 API 回傳、DB 寫入、頁面顯示的資料一致
3. **流程順暢性** — 斷言操作流程連貫：頁面跳轉正確、loading 狀態出現→消失、toast/feedback 即時顯示、表單提交後正確導向下一步
4. **跑 E2E 直到綠燈** — 測試失敗代表功能有 bug，修到過為止
5. **截圖自檢** — Playwright 截圖確認元素存在、佈局不破版
6. **全過才 commit**
...
1. **收尾稽核 subagent（標「✅ 完成」之前的強制步驟）**
   ...
   稽核結果寫入 `wave-{id}.md`「🕵️ 稽核結果」區塊。**有 ❌ 或漏項 → 打回修復，修完重派稽核**，不得標「✅ 完成」。
