...
> **CRITICAL: 每一波（dev / general / patch 全模式）掃完工作項後一律執行 `/grill-me`，無例外、無自我豁免。**「全部都是明確 bug fix」不是跳過理由——歷史教訓是：跳過訪談的波最常做出「做完了但不是使用者要的」的東西。（2026-07-20 使用者裁定改為無條件）

**Grill 的聚焦（依波型調整深度，不調整有無）：**
- 新功能／架構波 → 完整壓力測試（方案、邊界、風險、驗收標準）
- 回饋批次／patch 波 → 聚焦快收斂：與既有共識矛盾的項、範圍認定（哪些不修＋理由）、優先序、模糊描述的真實意圖——通常數題即收斂，不拖長
...
**格式：** 依 mode 照 `references/templates.md`「Goal Condition 模板」節逐字填入（dev 八條＋約束 / general 四條 / patch 四條），{id} 與約束代入本波實值。
