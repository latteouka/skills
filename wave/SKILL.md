...
1. **Phase 0 Align 問題**——「這波有新素材要帶入嗎？」（`/wave` 帶參數時跳過，此時全程零規劃停點）
...
**Turn 結束前自檢：** 結束 turn 前檢查最後一段輸出——若是計畫、問題、下一步清單、或「我接下來會…」的承諾 → 立刻用 tool call 做掉，不准就此結束 turn。
...
(9) 🕵️ 收尾稽核 subagent 已跑且全 ✅，結果貼在 wave-{id}.md 稽核結果區
(10) 全程遵守唯一停點制——除 Align 與範圍確認外未曾停下等待輸入
...
> **CRITICAL: 以下五條在整個執行期持續生效。長 session、compaction、跨 session 續跑都不豁免。**
...
**4. Re-grounding 協議**
- 每完成一項 commit + 更新 dashboard 時，同時重讀 goal condition 與 dashboard 未完成清單，輸出一行狀態（例：`3/7 完成，下一項 🤖-4`）再繼續
