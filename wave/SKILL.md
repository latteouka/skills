---
name: wave
description: 規劃並啟動一波開發。讀 requirements → 偵測是否需要 align 新素材 → 分類自主/人測項 → 產出 living dashboard + session prompt + goal condition。支援多波並行（各波獨立 worktree + dashboard）。觸發詞：/wave、開新波、新一波、plan wave、啟動開發。子指令：/wave status（全局概覽）、/wave drop {id}（放棄一波）。
