...
- UI 工作項必含「重用檢查」：動手前先盤點既有共用元件與同類 view 既定 pattern，重用優先、新建必須建成共用元件、禁止 inline 一次性樣式（決策順序見 `references/ui-fragmentation-audit.md`「元件重用紀律」節）；
  合約結果欄寫 `REUSE: 用了 {既有清單}；新建 {N}（理由）`——缺 REUSE 行＝合約不完整。防每波各自發明樣式造成的漂移累積
...
3. **未涵蓋的寫進收件匣** — 沒有對應工作項的決策 append 到 `.claude/dev/inbox.md`（未裝 intake kit 的專案寫進本波 dashboard 的「未涵蓋決策」區），每筆記錄：
   - `raw`：決策內容一句話 ＋ 為何本波不做（scope cut / blocker / 依賴未就緒）
...
**❓/需裁定項的處理（必做）：** 掃描出的每個 ❓ 未決項、以及 Phase 1 Ground 對不到的每個 ❓ 項，必須做成 Phase 4 AskUserQuestion 的**獨立問題**（選項 = 各方案／最似位置推測，推薦方案標 Recommended）——使用者就在停點上，裁定成本最低。Grounding ❓ 項裁定後立即把確認的對應 append 回 `feature-map.md` 別名欄。**禁語：「先跳過」「下波再做」「待裁定後可追加」**。使用者裁定 → 該項排入本波；使用者明選「延後」→ 才 append 進 `.claude/dev/inbox.md`（`from: grill:{題目}`）。不得由 Claude 自行決定延後。
...
1. **Artifact 欠繳掃描（機械掃，缺哪補哪，收尾稽核前）**——逐工作項核對「欠的行」是否已落檔：
   行為變更項有 INTENT、bug fix 項有 TWINS、UI 項有 REUSE、外向動作有 AUTH、文件規定但本波未執行的
   後續動作（deploy/restart/發佈/通知）有 `PENDING: <動作> — 待使用者授權`。欠而缺 →
   現在補齊再往下；PENDING 項同時醒目列入完成報告（與未涵蓋事項揭露並列）——merge 後需
