...
/wave
  ...
  │     重大結論（根因/架構/安全）採信前過三鏡頭抗辯（skeptic/red-team/simplifier）
  │
  └─ 收尾
...
|-------|------|------|-----------|
| **wave** | 波次規劃與啟動（核心） | 原創 | — |
| grill-me | 逐題壓力測試計畫 | 原創 | Phase 2.5 |
| grill-with-docs | 壓力測試 + 同步更新文件 | 原創 | Phase 2.5 |
| insecure-defaults | 安全審計：不安全預設值 | vendored | dev-mode 閘門 |
| sharp-edges | 安全審計：API 銳角 | vendored | dev-mode 閘門 |
| static-analysis | 靜態分析（semgrep/codeql/sarif） | vendored | dev-mode 閘門 |
| ui-test | 對抗性 UI 測試 | vendored | dev-mode 閘門 |
| wcag-accessibility-audit | WCAG 無障礙審計 | vendored | dev-mode 閘門 |
| nielsen-heuristics-audit | Nielsen 啟發式評估 | vendored | dev-mode 閘門 |
| ux-audit-rethink | UX 全面重新檢視 | vendored | dev-mode 閘門 |
...
`vendors.lock` 記錄每個 skill 上次同步的上游 SHA 與本地內容 hash——apply 前若偵測到本地被手動改過會跳過並警告（要強蓋加 `--force`）。**原創 skill（wave / grill-me / grill-with-docs）不在 `vendors.conf` 內，永遠不受同步影響。**

授權：`trailofbits/*` 為 CC-BY-SA-4.0（需保留署名），`mastepanoski/*` 為 MIT。

Wave 的三鏡頭抗辯流程參考自 [Miguok/fable-harness](https://github.com/Miguok/fable-harness)（MIT），已內嵌進 wave SKILL.md、無需另裝。
