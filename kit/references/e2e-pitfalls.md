...
新增或修改斷言後，必須先把期望故意改成**相反**的值，確認測試會紅；恢復正確期望後再確認綠。只驗綠燈不證明 locator、等待條件真的驗到目標。

### 反證步驟

1. 寫出正確、具體且 scope 明確的 locator 與期望。
2. 暫時改成相反期望，執行測試，確認斷言紅燈。
3. 還原正確期望，重跑並確認綠燈。

### 實證案例

- **固定等待會假通過**：`urlUnchanged` 曾只觀察 1500ms；dev server 慢導航數秒後才落 URL，測試先綠、實際仍跳頁。現行 CTA outcome helper 預設觀察 5000ms，並攔截 document／RSC 導航請求；不能把「短暫 URL 沒變」當成「永遠不導航」。
- **locator 未 scope 會驗錯元素**：同文字或同 role 同時出現在側欄與目標區時，未 scope 的 locator 可能抓到錯元素；再加上期望值過寬，測試仍會綠。應 scope 到具名容器、dialog 或目標區塊，並以反相期望確認抓錯元素時會紅。

### CTA 逐顆復位

`assertCtaOutcomes` 每顆 CTA 都先呼叫 `resetState(page)`，再 `goto(startUrl)`。URL 復位不會清除 localStorage、cookie 或 DB 的持久狀態；呼叫端必須用 hook 清理該 view 的狀態，確保每顆 CTA 從相同前提開始。
