...
| 權威 gate | 不跑 `收尾` phase(收尾鎖與 preauth 語意屬 merge 權威) | gate wrapper(nonce footer+OID receipt) |
...
- **基線 gate 與品質閘門**:一律 `bash scripts/hooks/wave-gate.sh baseline`(不取跨波收尾鎖、不查 base 新鮮度——那兩件事由 supervisor 的權威 gate+auto-regate 承擔)。`收尾` phase 禁用:branch PASS 會 preauth 持鎖,supervisor 的 PR merge 不會走 main re-gate 收口,鎖會洩漏讓下一波空等。
